...
Ex1 <- filter(NTL.phys.data, filter(year4 == 1999))
...
# (hint: you will need to filter for depth==0.0). 
Ex4 <- Ex3 %>% 
  filter(depth == 0) %>% 
  summarise(meanT = mean(temperature_C))
...
Ex6 <- NTL.phys.data %>% 
  pivot_wider(
    names_from = factor('depth'),
    values_from = temperature_C,
    id_cols = c('lakeid','lakename','sampledate')
  )
